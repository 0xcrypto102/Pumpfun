...
use crate::{
    constants::{
        BONDING_CURVE, GLOBAL_STATE_SEED, INITIAL_TOKEN_PRICE, PRICE_INCREASE_RATE, SOL_VAULT_SEED, UNITS_PER_TOKEN
    },
    error::*,
    events::*,
...
pub fn buy(ctx: Context<Buy>, sol_cost: u64, min_token_output: u64) -> Result<()> {
    ...
    require!(
        ...
        ApeLolCode::BondingCurveComplete
    );
    require!(
        accts.fee_recipient.key() == accts.global.fee_recipient,
        ApeLolCode::UnValidFeeRecipient
    );
    ...
    transfer(cpi_ctx.with_signer(signer), token_amount)?;
    ...
    let macp = ((accts.bonding_curve.virtual_sol_reserves as u128)
        * (accts.bonding_curve.token_total_supply as u128)
        / (accts.bonding_curve.real_token_reserves as u128)) as u64;
    msg!("macp:{}", macp);
    let percentage = ((accts.bonding_curve.real_token_reserves as u128) * (100 as u128)
        / (accts.bonding_curve.token_total_supply as u128)) as u64;
